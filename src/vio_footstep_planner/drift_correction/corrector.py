...
class DriftCorrector:
    ...
    def __init__(self, loop_closure_threshold=0.7, enable_pose_graph_optimization=True,
                 min_loop_interval=30, optimization_frequency=10):
        ...
        self.loop_detector = LoopClosureDetector(
            similarity_threshold=loop_closure_threshold,
            min_loop_interval=min_loop_interval
        )
    ...
    def add_pose(self, pose: np.ndarray, image: Optional[np.ndarray] = None):
        ...
        if image is not None and self.current_id >= self.min_loop_interval:
            ...
            loop_result = self.loop_detector.detect_loop_closure(
                image, pose, self.current_id
            )
            ...
            if loop_result is not None:
                ...
                print(f"Loop closure detected: {matched_id} <-> {self.current_id} (confidence: {confidence:.2f})")
        ...
        if (self.enable_optimization and 
            len(self.loop_closures) > 0 and
            self.poses_since_optimization >= self.optimization_frequency):
    ...
    def get_statistics(self) -> Dict:
        ...
        """
        avg_drift = (self.cumulative_drift / self.total_drift_corrections 
                    if self.total_drift_corrections > 0 else 0.0)
