...
class PoseGraph:
    ...
    def _residual_function(self, x: np.ndarray, node_ids: List[int], fixed_node: int) -> np.ndarray:
...
class PoseGraphOptimizer:
    ...
    def add_odometry_edge(self, from_id: int, to_id: int, relative_transform: np.ndarray):
    ...
    def add_loop_closure_edge(
        self, from_id: int, to_id: int, relative_transform: np.ndarray, confidence: float = 0.5
    ):
