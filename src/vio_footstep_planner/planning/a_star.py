...
class AStarPlanner:
    ...
    def __init__(
        self, step_cost_fn: Optional[Callable] = None, heuristic_fn: Optional[Callable] = None
    ):
    ...
    def plan(
        ...
        valid_successors_fn: Optional[Callable] = None,
    ) -> Optional[List]:
        ...
        # Initialize
        start_node = Node(
            state=start, g_cost=0.0, h_cost=self.heuristic_fn(start, goal), parent=None
        )
        ...
        while open_set and iterations < max_iterations:
            ...
            for successor_state in successors:
                ...
                step_cost = self.step_cost_fn(current.state, successor_state, obstacle_map)
                tentative_g = current.g_cost + step_cost

                # Check if this path is better
                if successor_state not in g_costs or tentative_g < g_costs[successor_state]:

                    g_costs[successor_state] = tentative_g
                    h_cost = self.heuristic_fn(successor_state, goal)

                    successor_node = Node(
                        state=successor_state, g_cost=tentative_g, h_cost=h_cost, parent=current
                    )
