...
class ReachabilityChecker:
    ...
    def is_reachable(self, current_footstep: np.ndarray, 
                    next_footstep: np.ndarray,
                    foot_id: Optional[int] = None) -> bool:
    ...
    def get_reachable_region(self, current_footstep: np.ndarray,
    ...
    def check_stability(self, footsteps: np.ndarray) -> bool:
        ...
        if (x_min + margin <= com[0] <= x_max - margin and
            y_min + margin <= com[1] <= y_max - margin):
            return True
        
        return False
    
    def get_valid_footstep_positions(self, body_pose: np.ndarray,
                                    foot_id: int) -> np.ndarray:
