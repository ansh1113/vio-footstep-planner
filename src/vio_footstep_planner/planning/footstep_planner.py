...
class FootstepPlanner:
    ...
    def __init__(self, robot_model: str, config: Optional[Dict] = None):
        ...
        self.astar_planner = AStarPlanner(
            step_cost_fn=self._compute_step_cost, heuristic_fn=self._compute_heuristic
        )
    ...
    def plan(
        self, current_pose: np.ndarray, goal: List[float], obstacle_map: Optional[np.ndarray] = None
    ...
    def _generate_successors(
        self, state: tuple, obstacle_map: Optional[np.ndarray] = None
    ) -> List[tuple]:
        ...
        for length in step_lengths:
            for d_angle in step_angles:
                ...
                # Check reachability
                if not self.reachability_checker.is_reachable(current_footstep, next_footstep):
                    continue
    ...
    def _compute_step_cost(
        self, state1: tuple, state2: tuple, obstacle_map: Optional[np.ndarray] = None
    ) -> float:
    ...
    def _is_collision(self, footstep: np.ndarray, obstacle_map: np.ndarray) -> bool:
    ...
    def _get_terrain_cost(self, state: tuple, obstacle_map: np.ndarray) -> float:
    ...
    def get_plan_statistics(self) -> Dict:
        ...
        if len(self.last_plan) == 0:
            return {"num_steps": 0, "total_distance": 0.0, "avg_step_length": 0.0}

        # Compute statistics
        total_distance = 0.0
        for i in range(len(self.last_plan) - 1):
            step_dist = np.linalg.norm(self.last_plan[i + 1][:2] - self.last_plan[i][:2])
