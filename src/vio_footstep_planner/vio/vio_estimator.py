...
class VIOEstimator:
    ...
    def process_imu(self, timestamp: float, acc: np.ndarray, gyro: np.ndarray):
        ...
        """
        self.imu_buffer.append({"timestamp": timestamp, "acc": acc.copy(), "gyro": gyro.copy()})

        # Keep only recent IMU data (last 2 seconds)
        if len(self.imu_buffer) > 1000:
            self.imu_buffer = self.imu_buffer[-1000:]

    def process_image(self, timestamp: float, features: np.ndarray, feature_ids: List[int]) -> bool:
        ...
        imu_measurements = self._get_imu_between_frames(self.last_image_timestamp, timestamp)
        ...
        self.position = (
            self.position + self.velocity * delta_t + 0.5 * self.gravity * delta_t**2 + R @ delta_p
        )
